...
public class SecurityConfig {
    ...
    public SecurityFilterChain internalApiFilterChain(HttpSecurity httpSecurity) throws Exception {
        httpSecurity.securityMatcher("/internal/**")
                .authorizeHttpRequests(request -> request.requestMatchers("/internal/api/**")
                        .hasRole(globalProperties.getSerpServiceRole())
    ...
    public SecurityFilterChain publicApiFilterChain(HttpSecurity httpSecurity) throws Exception {
        httpSecurity.securityMatcher("/purchase-service/api/**")
                .authorizeHttpRequests(request -> {
                    ...
                    urlProperties.getProtectedUrls().forEach(url -> {
    ...
    public JwtDecoder jwtDecoder() {
        ...
        return NimbusJwtDecoder
                .withJwkSetUri(keycloakConfig.getJwkSetUri())
                .build();
    }
    ...
    public JwtAuthenticationConverter jwtAuthenticationConverter() {
        ...
        converter.setJwtGrantedAuthoritiesConverter(jwt -> {
            ...
            Map<String, Object> realmAccess = jwt.getClaimAsMap(keycloakConfig.getRealmAccess());
            if (realmAccess != null && realmAccess.containsKey(keycloakConfig.getRolesAttribute())) {
                Collection<String> realmRoles = (Collection<String>) realmAccess.get(keycloakConfig.getRolesAttribute());
                realmRoles.forEach(
                        role -> authorities.add(new SimpleGrantedAuthority(globalProperties.getRolePrefix() + role)));
            }

            Map<String, Object> resourceAccess = jwt.getClaimAsMap(keycloakConfig.getResourceAccess());
            if (resourceAccess != null) {
                resourceAccess.values().forEach(resource -> {
                    if (resource instanceof Map) {
                        Map<String, Object> resourceMap = (Map<String, Object>) resource;
                        if (resourceMap.containsKey(keycloakConfig.getRolesAttribute())) {
                            Collection<String> resourceRoles = (Collection<String>) resourceMap
                                    .get(keycloakConfig.getRolesAttribute());
                            resourceRoles.forEach(role -> authorities
                                    .add(new SimpleGrantedAuthority(globalProperties.getRolePrefix() + role)));
                        }
                    }
                });
            }
        ...
        converter.setJwtGrantedAuthoritiesConverter(jwt -> {
            ...
            if (isValidServiceClient(azp) && isValidServiceClient(clientId)) {
                authorities.add(new SimpleGrantedAuthority(
                        globalProperties.getRolePrefix() + globalProperties.getSerpServiceRole()));
            }
