...
export const OrderForm: React.FC<OrderFormProps> = ({
  ...
  const [supplierId, setSupplierId] = useState(
    initialSupplierId || order?.fromSupplierId || ''
  );
  ...
  // Fetch pre-selected supplier if supplierId is provided
  const { data: preSelectedSupplierResponse } = useGetSupplierQuery(
    supplierId,
    { skip: !supplierId || isEditMode || !!supplierSearch }
  );

  // Fetch suppliers and products
  const { data: suppliersResponse } = useGetSuppliersQuery({
    filters: { query: supplierSearch },
  ...
  const validateForm = (): boolean => {
    ...
    if (!saleChannel) {
      newErrors.saleChannel = 'Kênh bán hàng là bắt buộc';
    }
    ...
    if (deliveryBeforeDate && deliveryAfterDate) {
      ...
      if (before < after) {
        newErrors.deliveryBeforeDate =
          'Ngày giao hàng trước phải sau ngày giao hàng sau';
      }
    }
  ...
  const selectedSupplier =
    suppliers.find((s) => s.id === supplierId) || preSelectedSupplier;
            ...
            <CardContent className='space-y-4'>
              ...
              <div className='space-y-2'>
                ...
                <Input
                  ...
                  placeholder='ví dụ: Đơn nhập hàng tháng 12'
                />
              </div>
              ...
              <div className='space-y-2'>
                <Label htmlFor='saleChannel'>
                  Kênh bán hàng <span className='text-destructive'>*</span>
                </Label>
                <select
                  id='saleChannel'
                  value={saleChannel}
                            ...
                            <div>
                              ...
                              {product?.quantityAvailable !== undefined && (
                                <p className='text-xs text-muted-foreground mt-1'>
                                  Có sẵn: {product?.quantityAvailable}
                                </p>
                              )}
                            </div>
                            <div>
                ...
                {deliveryBeforeDate && (
                  <div className='pt-4 border-t'>
                    ...
                    <p className='text-sm font-medium'>
                      {new Date(deliveryBeforeDate).toLocaleDateString('vi-VN')}
                    </p>
                  </div>
                )}
              </CardContent>
